...
// Wiring test for the login() server action: that the limiter gates the credential
...
vi.mock('@/server/services/user.service', () => ({
  userService: { verifyLogin: m.verifyLogin },
}));
...
describe('login() wiring', () => {
  test('a blocked IP is refused without checking credentials or delaying', async () => {
    ...
    expect(m.verifyLogin).not.toHaveBeenCalled();
  ...
  test('wrong credentials delay, return the generic error, and record a failure', async () => {
    m.check.mockReturnValue({ blocked: false, retryAfterSec: 0 });
    m.verifyLogin.mockResolvedValue(null);
  ...
  test('valid credentials reset the limiter and set the session cookie', async () => {
    m.check.mockReturnValue({ blocked: false, retryAfterSec: 0 });
    m.verifyLogin.mockResolvedValue({ id: 'user-1' });
